...
const numericStyles = ["fontSize", "margin", "padding"]; // Функция приведения параметра к необходимому типу значения

export default ((el, appendix = "rem") => {
  // Если параметра нет в списке возвращаем все как было
  if (!numericStyles.includes(el.name)) return el; // Если есть апендикс добавляем
